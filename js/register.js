...
async function checkEmailAvailability(email) {
    ...
    try {
        ...
        const modal = new bootstrap.Modal(document.getElementById('idCheckModal'));
        modal.show();
    } catch (error) {
        console.error(error);
        alert(error.message);
...
const validators = {
    username: (value) => {
        return value.length >= 2 && value.length <= 20 && /^[A-Za-z가-힣]+$/.test(value)
            ? ''
...
document.getElementById('password').addEventListener('input', (event) => {
    ...
    validateField('passwordConfirm', document.getElementById('passwordConfirm').value, {
...
/* 회원가입 클릭 시 */
document.querySelector('form[name="register"]').addEventListener('submit', async function (event) {
